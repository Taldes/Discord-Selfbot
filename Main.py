...
ctypes.windll.kernel32.SetConsoleTitleW(f'[Alucard #{SELFBOT.__version__}] | Loading...')
...
def startprint():
    ...
    print(f'''{Fore.RESET}
    	{Fore.RED}
                       ...
                       {Fore.CYAN}Creator: {Fore.BLUE}Taldes#1337
    '''+Fore.RESET)
...
async def geoip(ctx, *, ipaddr: str = '1.8.4.5'): # b'\xfc'
    await ctx.message.delete()
...
@Alucard.command()
async def masskick(xfc): # b'\xfc'
    await xfc.message.delete()
    for user in list(xfc.guild.members):
        try:
            await user.kick()
        except:
            pass    
